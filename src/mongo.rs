...
pub fn collection<T: Serialize + Send + Sync>(database: &mongodb::Database, collection_name: &str) -> Collection<T> {
...
pub async fn insert_many<T: Serialize + Send + Sync>(collection: &Collection<T>, docs: impl IntoIterator<Item = T>) {
    let insert_result = collection.insert_many(docs).await.unwrap();
    println!("Inserted documents with ids: {:?}", insert_result.inserted_ids);
}
